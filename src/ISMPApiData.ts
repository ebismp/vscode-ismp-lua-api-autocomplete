...
export default class ISMPApiData {

    private classes: ISMPTypeMap
    private defines: ISMPTypeMap

    constructor(private dataPath: string) {
        this.loadData(dataPath)
    }

    public findType(words: string[]): ISMPType {
        if (words.length === 0) {
    ...
    private loadDataFile(fileName: string): ISMPTypeMap {
